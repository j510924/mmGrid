...
!(function ($) {
    ...
    MMGrid.prototype = {
        ...
        _rowObject: function (item, rowIndex) {
            var opts = this.opts;
            var expandCols = this._expandCols(opts.cols);
            var leafCols = this._leafCols();
            ...
            if ($.isPlainObject(item)) {
                ...
                for (var colIndex = 0, leafColsLen = leafCols.length; colIndex < leafColsLen; colIndex++) {
                    var col = leafCols[colIndex];
                    var index = $.inArray(col, expandCols);
                    var cls = this._genColClass(index);
        ...
        _populate: function (items) {
            ...
            if (items && items.length !== 0 && opts.cols) {
                ...
                var $tbody = $('<tbody>');
                for (rowIndex = 0; rowIndex < itemsLen; rowIndex++) {
                    var item = items[rowIndex];
                    $tbody.append(this._rowObject(item, rowIndex));
                }
                $body.empty().html($tbody);
            ...
            $.ajax({
                ...
                try {
                    if (data.resultCode === '200') {
                        ...
                        if (data && $.isArray(list)) {
                            data = $.extend(args, data);
                        }
                        that.$body.triggerHandler('loadSuccess', data);
                    } else {
                        opts.notify(data.resultMessage);
                        that._hideLoading();
                        that._showLoadError();
